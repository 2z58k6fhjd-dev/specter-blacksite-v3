...
const scene=new THREE.Scene();scene.background=new THREE.Color(0x020504);scene.fog=new THREE.FogExp2(0x020605,.025);
const camera=new THREE.PerspectiveCamera(72,innerWidth/innerHeight,.04,120);camera.position.set(0,1.72,7);
...
const emergency=new THREE.PointLight(0xff2600,8,18,2);emergency.position.set(0,2.8,-16);scene.add(emergency);

const world=buildWorld(scene);
const assets=new AssetManager(CONFIG);
const weapons=new WeaponSystem(camera,CONFIG);
...
const keys={};const raycaster=new THREE.Raycaster();const clock=new THREE.Clock();
const flashlight=new THREE.SpotLight(0xe8fff3,75,22,Math.PI/7,.42,1.4);flashlight.position.set(.18,-.10,-.12);flashlight.target.position.set(0,0,-8);camera.add(flashlight,flashlight.target);

const toast=t=>{const m=document.getElementById('message');m.textContent=t;m.style.opacity=1;clearTimeout(toast.id);toast.id=setTimeout(()=>m.style.opacity=0,1600)};
function hud(){
...
function movement(dt){
  const fi=(keys.KeyW?1:0)-(keys.KeyS?1:0),si=(keys.KeyD?1:0)-(keys.KeyA?1:0);
  const forward=new THREE.Vector3();camera.getWorldDirection(forward);forward.y=0;forward.normalize();
  const right=new THREE.Vector3().crossVectors(forward,new THREE.Vector3(0,1,0)).normalize();
  const v=forward.multiplyScalar(fi).add(right.multiplyScalar(si));
  weapons.moving=v.lengthSq()>.01;if(v.lengthSq()>1)v.normalize();
...
function restorePower(){
  if(powerOn)return;powerOn=true;world.lights.forEach((l,i)=>setTimeout(()=>l.intensity=16,i*110));emergency.intensity=.8;
  document.getElementById('objective').textContent='OBJECTIVE: ELIMINATE VOLK TEAM';toast('FACILITY POWER RESTORED');hud();
}
function interact(){
  raycaster.setFromCamera(new THREE.Vector2(),camera);const h=raycaster.intersectObject(world.switchGroup,true)[0];
  if(h&&h.distance<2.7)restorePower();
}
function shoot(){
  const now=performance.now();if(!weapons.shoot(now))return;
  raycaster.setFromCamera(new THREE.Vector2(),camera);
  const hits=raycaster.intersectObjects([...enemies.enemies,...world.collision],true);
  if(hits[0]){
    const e=hits[0].object.userData.enemy;
    if(e&&!e.userData.dead){e.userData.health-=weapons.current==='rifle'?38:28;if(e.userData.health<=0){e.userData.dead=true;e.rotation.z=Math.PI/2;kills++;toast('HOSTILE NEUTRALIZED')}}
  }
  hud();
}
...
addEventListener('mousedown',e=>{if(e.button===0){fireHeld=true;shoot()}if(e.button===2)weapons.aiming=true});
addEventListener('mouseup',e=>{if(e.button===0)fireHeld=false;if(e.button===2)weapons.aiming=false});
addEventListener('contextmenu',e=>e.preventDefault());

document.getElementById('startButton').onclick=()=>{started=true;document.getElementById('startPanel').style.display='none';controls.lock()};
...
const rifle=assets.clone('ar15');if(rifle){assets.normalize(rifle,1.28);weapons.install('rifle',rifle)}
const pistol=assets.clone('m9');if(pistol){assets.normalize(pistol,.48);weapons.install('pistol',pistol)}
enemies.spawnAll();hud();

function animate(){
  requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.05),t=clock.elapsedTime;
  if(started){movement(dt);weapons.update(dt,t);enemies.update(dt,t,controls.object);if(fireHeld&&weapons.current==='rifle'&&weapons.fireMode==='auto')shoot();
    raycaster.setFromCamera(new THREE.Vector2(),camera);const h=raycaster.intersectObject(world.switchGroup,true)[0];
