#!/usr/bin/env python3
"""Build reproducible 1024px browser textures for SPECTER's Medium tier.

The source models and project-generated PBR maps remain untouched. This script
creates a parallel ``assets/medium-textures/`` tree with the same relative
texture paths, allowing the loader to choose real 1K images before the 2K/4K
source images are decoded on Performance/Balanced startup paths.
"""
...
def main() -> None:
    ...
    for source in source_files():
        ...
        records.append({
            ...
            "file": f"assets/medium-textures/{relative.as_posix()}",
    ...
    for existing in (OUTPUT.rglob("*") if OUTPUT.exists() else []):
    ...
    manifest = {
        "schemaVersion": 1,
        "assetSet": "SPECTER medium 1K texture tier",
        "generatedBy": "scripts/build-medium-textures.py",
    ...
    (OUTPUT / "README.md").write_text(
        ...
        "This tree is a reproducible 1024px-max derivative of bundled runtime textures. "
        "It is generated by `scripts/build-medium-textures.py` and selected before "
        "2K/4K source decode on Performance/Balanced or a custom Medium startup. "
    ...
    print(f"Built {len(records)} medium textures ({runtime_bytes / (1024 * 1024):.2f} MiB).")
